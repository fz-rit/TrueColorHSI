...
from Accessories_from_colour import get_illuminant_spd_and_xyz
...
def make_compare_plots(images: list, 
    ...
    axes[1].axis('off')
    axes[1].set_title(f'{subplot_title}(Intensity normalized)')

    axes[2].imshow(images[2])
    axes[2].axis('off')
    axes[2].set_title(f'{subplot_title}(CLAHE on L channel)')
...
def vanilla_visualization(hyperspectral_data: spectral.image.ImageArray,
    ...
    display_images = [viz_simple, viz_norm, viz_clahe_on_L]
    make_compare_plots(images=display_images, 
                       suptitle='Direct visualization of hyperspectral images', 
                       subplot_title='RGB ',
...
def colorimetric_visualization(hyperspectral_data: spectral.image.ImageArray, 
    ...
    std_wavelengths, D65_values, xyz = get_illuminant_spd_and_xyz()
    ...
    display_images = [SRGB_image, SRGB_norm, SRGB_clahe_on_L]
    make_compare_plots(images=display_images,
                        suptitle='Colorimetric-based visualization of hyperspectral images',
                        subplot_title='Colorimetric-based visualization ',
                        saveimages=saveimages,
                        savefolder=savefolder)
...
if __name__ == "__main__":
    ...
    input_folder = Path(r"G:\My Drive\projects_with_Dave\for_Fei\Data\Symeon")
    ...
    colorimetric_visualization(hyperspectral_data, saveimages, output_folder)